...
static cl::opt<std::string> ProfileInfoFilename("profile-info-file",
    cl::init("llvmprof.out"), cl::value_desc("filename"),
...
void LoaderPass::readEdgeOrRemember(Edge edge, Edge &tocalc, unsigned &uncalc,
    double &count) {
  double w;
...
void LoaderPass::recurseBasicBlock(const BasicBlock *BB) {
  ...
  if (BBisUnvisited.find(BB) == BBisUnvisited.end())
    return;
  BBisUnvisited.erase(BB);
  if (!BB)
    return;

  for (succ_const_iterator bbi = succ_begin(BB), bbe = succ_end(BB); bbi != bbe;
      ++bbi) {
    recurseBasicBlock(*bbi);
  }
  for (const_pred_iterator bbi = pred_begin(BB), bbe = pred_end(BB); bbi != bbe;
      ++bbi) {
    recurseBasicBlock(*bbi);
  }
...
void LoaderPass::readEdge(ProfileInfo::Edge e, std::vector<unsigned> &ECs) {
  ...
  if (Counters.size() > 0) {
    ...
    for (Module::iterator F = M.begin(), E = M.end(); F != E; ++F) {
      ...
      while (SpanningTree.size() > 0) {
        ...
        BBisUnvisited.clear();
        for (std::set<Edge>::iterator ei = SpanningTree.begin(), ee =
            SpanningTree.end(); ei != ee; ++ei) {
...
int main() {
  ...
  error_code err = MemoryBuffer::getFile("./eg.i.bc", buffer);
